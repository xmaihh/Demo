...
/**
 * Created by Administrator on 2018/3/13. Instruction:Rxjava2.x实现定时器
 */
    ...
    public static void timer(long milliseconds, final IRxNext next) {
        Observable.timer(milliseconds, TimeUnit.MILLISECONDS).observeOn(AndroidSchedulers.mainThread()).subscribe(new Observer<Long>() {
            ...
            public void onNext(@NonNull Long number) {
                if (next != null) next.doNext(number);
            }

            @Override
            public void onError(@NonNull Throwable e) { /*取消订阅*/
                cancel();
            }

            @Override
            public void onComplete() { /*取消订阅*/
                cancel();
            }
        });
    }
